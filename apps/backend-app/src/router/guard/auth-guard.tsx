...
export function AuthGuard({ children }: AuthGuardProps) {
	...
	useEffect(() => {
      ...
      console.log('routes---', routes)
		 ...
		 * 2. User info is available (otherwise try to fetch by cookie session)
		 * 3. Unable to obtain user information and route information
		 *
		 */
		if (!whiteRouteNames.includes(pathname) && !isAuthorized) {
	...
	/**
	 * @zh 匹配 login 路由时：无用户信息保留登录页，有用户信息则跳首页
	 *
	 * @en For login route: keep login page without user info; redirect home when user info exists
	 */
	if (pathname === loginPath) {
		if (!isAuthorized) {
			hideLoading();
