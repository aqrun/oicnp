...
//! - **CSS 文件**: 通过 JS 中的 `import './style.css'` 加载
//!   - 不需要单独的 `<link>` 标签
//!   - CSS 文件会被 Vite dev server 自动处理并支持 HMR
//!
//! ### Release 模式（生产环境）
//!
//! - **JS 文件**: 从 `manifest.json` 自动读取编译后的路径
//!   - 入口文件: 优先查找 `isEntry: true` 的条目
//!   - 如果没有，查找常见入口文件名（`main.js`, `main.tsx`, `index.js` 等）
//!   - 自动包含所有静态导入的依赖文件
//! - **CSS 文件**: 从 `manifest.json` 自动读取
//!   - 优先从入口文件的 `css` 字段获取
//!   - 如果没有，查找独立的 CSS 条目
//!
//! ## 项目结构要求
