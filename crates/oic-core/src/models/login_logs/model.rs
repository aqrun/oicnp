...
impl ActiveModelBehavior for LoginLogActiveModel {}

#[async_trait::async_trait]
impl ModelCrudHandler for LoginLogModel {
    type DataModel = Self;
    type FilterParams = LoginLogFilters;
    type CreateReqParams = CreateLoginLogReqParams;
    type UpdateReqParams = UpdateLoginLogReqParams;
    type DeleteReqParams = DeleteLoginLogReqParams;
    ...
    async fn find_by_id(db: &DatabaseConnection, id: i64) -> ModelResult<Self> {
        ...
        let item = LoginLogEntity::find()
            .filter(LoginLogColumn::Id.eq(id))
            .one(db)
            .await?;
    ...
    async fn find_list(db: &DatabaseConnection, params: &Self::FilterParams) -> ModelResult<(Vec<Self>, u64)> {
        ...
        if let Some(x) = params.id {
            if x > 0 {
                q = q.filter(LoginLogColumn::Id.eq(x));
            }
        }

        if let Some(x) = &params.login_name {
            if !x.is_empty() {
                q = q.filter(LoginLogColumn::LoginName.contains(x));
            }
        }
    ...
    async fn create_multi(
        ...
        let mut logs: Vec<LoginLogActiveModel> = Vec::new();

        for item in params.iter() {
            let mut log = LoginLogActiveModel {
                ..Default::default()
            };
    
            item.update(&mut log);
            item.update_by_create(&mut log);
            logs.push(log);
        }
        
        let _ = LoginLogEntity::insert_many(logs).exec(&txn).await?;
        txn.commit().await?;

        Ok(String::from("批量login_log添加完成"))
    }
    ...
    async fn create(db: &DatabaseConnection, params: &Self::CreateReqParams) -> ModelResult<i64> {
        ...
        let mut item = LoginLogActiveModel {
            ..Default::default()
        };
    ...
    async fn delete_one(db: &DatabaseConnection, params: &Self::DeleteReqParams) -> ModelResult<i64> {
        ...
        let _res = LoginLogEntity::delete_by_id(id)
            .exec(db)
            .await?;
